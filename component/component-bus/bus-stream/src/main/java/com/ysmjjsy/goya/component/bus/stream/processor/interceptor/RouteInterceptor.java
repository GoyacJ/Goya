...
public class RouteInterceptor implements IEventInterceptor {
    ...
    public void intercept(EventContext context) {
        ...
        if (result.isDeserialized() && result.getEvent() != null) {
            ...
            IBusEvent event = result.getEvent();
     ...
     * @return 匹配的监听器列表
     */
    private List<BusEventListenerScanner.EventListenerMetadata> findMatchingListeners(IBusEvent event, EventScope scope) {
     ...
     * @return 是否匹配
     */
    private boolean matchesScopeAndCondition(IBusEvent event, EventScope scope,
