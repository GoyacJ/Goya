...
import com.ysmjjsy.goya.component.bus.stream.publish.IRemoteEventPublisher;
import com.ysmjjsy.goya.component.bus.stream.publish.MetadataAccessor;
import com.ysmjjsy.goya.component.bus.stream.transaction.EventTransactionSynchronization;
...
public class DefaultBusService implements IBusService {
    ...
    @Override
    public <E extends IBusEvent> void publishLocal(E event) {
        if (event == null) {
        ...
        try {
            StrategyExecute publisher = strategyChoose.choose("LOCAL");
    ...
    @Override
    public <E extends IBusEvent> void publishRemote(E event) {
        if (event == null) {
            throw new CommonException("Event cannot be null");
        }
    ...
    @Override
    public <E extends IBusEvent> void publishDelayed(E event, Duration delay) {
        if (event == null) {
            throw new CommonException("Event cannot be null");
        }
    ...
    @Override
    public <E extends IBusEvent> void publishOrdered(E event, String partitionKey) {
        if (event == null) {
            throw new CommonException("Event cannot be null");
        }
     ...
     */
    private <E extends IBusEvent> boolean checkAndHandleDegradation(
     ...
     * @return destination
     */
    private String buildDestination(IBusEvent event) {
     ...
     */
    private <E extends IBusEvent> MessageHeaders buildMessageHeaders(E event, Duration delay, String partitionKey) {
        // 获取或生成 traceId
     ...
     * @return 幂等键
     */
    private String generateIdempotencyKey(IBusEvent event) {
        try {
            // 序列化事件对象为 JSON
    ...
    @Override
    public <E extends IBusEvent> void publishInTransaction(E event, Runnable transactionCallback) {
        if (event == null) {
            throw new CommonException("Event cannot be null");
        }
