...
import com.ysmjjsy.goya.component.cache.multilevel.core.RemoteCache;
...
import com.ysmjjsy.goya.component.cache.multilevel.local.NoOpLocalCache;
import com.ysmjjsy.goya.component.cache.multilevel.remote.NoOpRemoteCache;
...
public class CacheFactory {
    ...
    public <K, V> GoyaCache<K, V> createCache(String cacheName, CacheSpecification spec) {
        ...
        try {
            ...
            CacheLevelEnum cacheLevel = spec.getCacheLevel();
            if (cacheLevel == null) {
                // 默认多级缓存
            ...
            GoyaCache<K, V> cache = new GoyaCache<>(cacheName, l1, l2, spec, cacheBloomFilter, refillManager,
    ...
    public <K, V> GoyaCache<K, V> createL1Only(String cacheName, CacheSpecification spec) {
        ...
        builder.cacheLevel(CacheLevelEnum.L1_ONLY);
    ...
    public <K, V> GoyaCache<K, V> createL2Only(String cacheName, CacheSpecification spec) {
        CacheSpecification.Builder builder = new CacheSpecification.Builder(spec);
        builder.cacheLevel(CacheLevelEnum.L2_ONLY);
        return createCache(cacheName, builder.build());
    }
    ...
    public <K, V> GoyaCache<K, V> createL1L2(String cacheName, CacheSpecification spec) {
        CacheSpecification.Builder builder = new CacheSpecification.Builder(spec);
        builder.cacheLevel(CacheLevelEnum.L1_L2);
        return createCache(cacheName, builder.build());
    }
