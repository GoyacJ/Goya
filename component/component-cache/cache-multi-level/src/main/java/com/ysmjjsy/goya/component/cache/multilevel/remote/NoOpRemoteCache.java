package com.ysmjjsy.goya.component.cache.multilevel.remote;
 ...
 * 空操作远程缓存实现
 *
 * <p>用于支持 L1_ONLY 缓存模式，所有操作都是空操作（不执行任何实际缓存操作）。
 *
 * <p><b>职责：</b>
 * <ul>
 *   <li>实现 {@link RemoteCache} 接口的所有方法</li>
 *   <li>所有方法返回 null 或空集合，不执行任何实际操作</li>
 *   <li>用于 L1_ONLY 模式，避免 GoyaCache 需要判断 L2 是否存在</li>
 * </ul>
 *
 * <p><b>使用场景：</b>
 * <ul>
 *   <li>L1_ONLY 缓存模式：只使用本地缓存，不需要远程缓存</li>
 *   <li>测试场景：需要模拟无远程缓存的环境</li>
 * </ul>
 *
 * @author goya
 * @since 2025/12/26
 */
@Slf4j
public class NoOpRemoteCache implements RemoteCache {
     ...
     * @param name 缓存名称
     */
    public NoOpRemoteCache(String name) {
        this.name = name;
    }
    ...
    @Override
    public CompletableFuture<Cache.ValueWrapper> getAsync(@NonNull Object key) {
        // 空操作：立即返回 null
