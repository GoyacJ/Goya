 ...
 * 默认缓存配置规范解析器实现
 *
 * <p>从 {@link MultiLevelProperties} 读取配置，解析为 {@link CacheSpecification}。
 * 支持配置缓存，提升解析性能。
 *
 * <p><b>职责：</b>
 * <ul>
...
public class DefaultCacheSpecificationResolver implements CacheSpecificationResolver {
     ...
     */
    public DefaultCacheSpecificationResolver(MultiLevelProperties properties) {
        if (properties == null) {
    ...
    private CacheSpecification doResolve(String cacheName) {
        // 1. 获取配置（特定配置或默认配置）
        MultiLevelProperties.CacheConfig config = properties.getCacheConfig(cacheName);
     ...
     */
    private TtlStrategy createTtlStrategy(MultiLevelProperties.TtlStrategyConfig strategyConfig) {
