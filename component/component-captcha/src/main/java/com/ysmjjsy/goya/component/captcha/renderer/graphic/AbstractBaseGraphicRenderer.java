...
public abstract class AbstractBaseGraphicRenderer extends AbstractGraphicRenderer {
    ...
    protected String[] getWordCharacters() {
        int number = this.getCaptchaProperties().graphics().length();
    ...
    protected String[] getCharCharacters() {
        int number = getCaptchaProperties().graphics().length();
        CaptchaCharacterEnum captchaCharacter = getCaptchaProperties().graphics().letter();
    ...
    private void drawBezierCurve(Graphics2D graphics) {
        drawColor(graphics);
        int x1 = 5;
        int y1 = RandomProvider.randomInt(5, getHeight() / BOUND);
        int x2 = getWidth() - 5;
        int y2 = RandomProvider.randomInt(getHeight() / BOUND, getHeight() - 5);
        int ctrlx1 = randomCtrlX();
        int ctrly1 = randomCtrlY();
        if (RandomProvider.randomInt(BOUND) == 0) {
            int ty = y1;
            y1 = y2;
            y2 = ty;
        }
    ...
    private void drawCharacter(Graphics2D graphics, String[] characters, Color[] colors, String benchmark, boolean isGif, int alpha) {
        ...
        // 字符的左右边距
        int fSp = (fW - (int) fontMetrics.getStringBounds(benchmark, graphics).getWidth()) / BOUND;
