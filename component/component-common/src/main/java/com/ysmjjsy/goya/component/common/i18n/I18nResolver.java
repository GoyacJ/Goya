 ...
 * 国际化解析器
 * <p>
 * 提供枚举类型的国际化消息解析功能，支持多语言切换。
 * <p>
 * 使用方式：
 * <pre>{@code
 * // 使用默认 Locale
 * String message = I18nResolver.resolveEnum(ResponseCodeEnum.OK);
 *
 * // 指定 Locale
 * String message = I18nResolver.resolveEnum(ResponseCodeEnum.OK, Locale.SIMPLIFIED_CHINESE);
 * }</pre>
 * <p>
 * 注意：
 * <ul>
 *   <li>如果 MessageSource 未初始化，会返回枚举的默认描述</li>
 *   <li>如果国际化消息不存在，会返回枚举的默认描述（降级策略）</li>
 *   <li>优先使用请求级别的 Locale（LocaleContextHolder），如果没有则使用系统默认 Locale</li>
 * </ul>
 *
 * @author goya
