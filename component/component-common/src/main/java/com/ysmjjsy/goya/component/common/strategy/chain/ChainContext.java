package com.ysmjjsy.goya.component.common.strategy.chain;
...
public class ChainContext<T> implements CommandLineRunner {
    ...
    private final Map<String, List<IChainHandler<T>>> chainContainer = new HashMap<>();

    /**
     * 执行责任链
     */
    public void executeChain(String chainKey, T requestParam) {
        List<IChainHandler<T>> handlers = chainContainer.get(chainKey);

        if (CollectionUtils.isEmpty(handlers)) {
            throw new CommonException(String.format("Chain [%s] is not defined.", chainKey));
        }

        for (IChainHandler<T> handler : handlers) {
    ...
    public void run(String... args) {
        Map<String, IChainHandler> beanMap = SpringContext.getBeanMapsOfType(IChainHandler.class);
