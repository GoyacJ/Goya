...
public class GoyaIoUtils {
    ...
    public static void write(OutputStream outputStream, byte[] data) {
        if (outputStream == null) {
            throw Exceptions.system(CommonErrorCode.SYSTEM_ERROR).userMessage("write output is null").build();
        }
        if (data == null) {
            return;
        }
    ...
    public static long copy(InputStream inputStream, OutputStream outputStream) {
        if (inputStream == null || outputStream == null) {
            throw Exceptions.system(CommonErrorCode.SYSTEM_ERROR).userMessage("input and write output is null").build();
        }
        try {
            byte[] buffer = new byte[4096];
            long totalBytes = 0;
