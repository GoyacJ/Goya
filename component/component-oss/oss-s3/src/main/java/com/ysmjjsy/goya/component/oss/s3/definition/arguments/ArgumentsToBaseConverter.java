...
public abstract class ArgumentsToBaseConverter<S extends BaseArguments, T extends S3Request> implements OssConverter<S, T> {
    ...
    public void prepare(S arguments, T request) {
        if (MapUtils.isNotEmpty(arguments.getExtraHeaders())) {
            arguments.getExtraHeaders().forEach((key, value) -> 
                request.putCustomRequestHeader(key, value));
        }

        if (MapUtils.isNotEmpty(arguments.getExtraQueryParams())) {
            arguments.getExtraQueryParams().forEach((key, value) -> 
                request.putCustomQueryParameter(key, value));
        }
    }
}
